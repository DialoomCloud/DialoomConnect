...
- **Dynamic Service Pricing Fix (August 1, 2025)**: Fixed service add-on prices to be dynamically fetched from admin configuration instead of hardcoded values. User profile pages now display real-time prices from database, ensuring admin panel changes are immediately reflected across all host profiles
