    ...
    >
      <CardContent className="p-6">
        ...
        <div className="space-y-3">
          ...
          {/* Languages */}
          {hostLanguages[host.id] && hostLanguages[host.id].length > 0 && (
            <div className="flex items-start text-gray-600 text-sm">
              ...
              <div className="flex flex-wrap gap-1">
                {hostLanguages[host.id].map((userLang: any) => {
                  const language = languages.find((l: any) => l.id === userLang.languageId);
                  if (!language) return null;
                  const className = userLang.isPrimary
                    ? 'bg-[hsl(188,100%,95%)] text-[hsl(188,100%,35%)] px-2 py-0.5 rounded-full text-xs font-medium'
                    : 'bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full text-xs';
                  return (
                    <span key={userLang.languageId} className={className}>
                      {language.name}
                    </span>
                  );
                })}
